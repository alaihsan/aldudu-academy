document.addEventListener('DOMContentLoaded', function() {
    ...
    const initializeDashboard = async () => {
        ...
        try {
            ...
            classGrid.innerHTML = `<p class="text-red-600 text-sm">Gagal memuat data awal.</p>`;
        }
    };

    const handleYearChange = async () => {
        updateContentTitle();
        classGrid.innerHTML = `<p class="text-gray-600">Memuat kelas...</p>`;
        try {
            const data = await apiRequest(`/api/courses/year/${document.getElementById('academic-year').value}`);
            renderClassCards(data.courses);
        } catch(error) {
            classGrid.innerHTML = `<p class="text-red-600 text-sm">Gagal memuat data kelas.</p>`;
        }
    };
        ...
        try {
            ...
            if (data.success) {
                ...
                if (generatedClassCode) {
                    generatedClassCode.querySelector('span').textContent = data.course.class_code;
                }
            ...
            if (errorEl) {
                errorEl.textContent = error.message || "Gagal menyimpan kelas.";
                errorEl.classList.remove('hidden');
            }
        }
    };
    ...
    const updateContentTitle = () => {
        ...
        if (titleEl) {
            titleEl.textContent = opt ? `${baseTitle} di Tahun Ajaran ${opt.text}` : baseTitle;
        }
    };
    ...
    const createAndAppendClassCard = (cls) => {
        ...
        body.innerHTML = `
            <p class="class-card-info">Wali Kelas: ${cls.teacher}</p>
            <p class="class-card-info">${cls.studentCount} Murid</p>
        `;
        
        // Footer
        const footer = document.createElement('div');
        footer.className = 'class-card-footer';
        footer.innerHTML = `<a href="/kelas/${cls.id}" class="btn btn-secondary block text-center">Masuk Kelas</a>`;
        ...
        if (cls.is_teacher) {
            ...
            const checkIconSVG = `<svg class="check-icon icon-hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>`;
            codeSection.innerHTML = `
                <p>Kode Kelas</p>
                <div class="class-card-code-wrapper">
                    <span class="class-card-code">${cls.class_code}</span>
                    <button class="copy-code-btn" title="Salin Kode">${copyIconSVG}${checkIconSVG}</button>
                </div>
            `;
    ...
    if (quizModal) {
        const showQuizModal = () => {
            ...
            quizNameInput.value = '';
            quizModalError?.classList.add('hidden');
        ...
        const hideQuizModal = () => quizModal.classList.add('hidden');
    ...
    const initializeApp = async () => {
        ...
        classGrid?.addEventListener('click', e => {
            ...
            if (copyBtn) {
                ...
                if (codeElement && copyIcon && checkIcon) {
        ...
        generatedClassCode?.addEventListener('click', () => {
            const span = generatedClassCode.querySelector('span');
            if (span) {
